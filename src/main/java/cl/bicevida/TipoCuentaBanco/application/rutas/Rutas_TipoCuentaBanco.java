package cl.bicevida.TipoCuentaBanco.application.rutas;

import cl.bicevida.TipoCuentaBanco.application.controladores.*;
...
import cl.bicevida.TipoCuentaBanco.domain.puertoSalida.*;
...
@Path("/api/tipo-cuentabanco")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@Transactional(Transactional.TxType.SUPPORTS)
public class Rutas_TipoCuentaBanco {

    @Inject
    Validator validator;

    @Inject
    PuertoSalida_BuscarTodos_TipoCuentaBanco buscarTodos_PuertoSalida;
    @Inject
    PuertoSalida_BuscarPorID_TipoCuentaBanco buscarPorID_PuertoSalida;
    @Inject
    PuertoSalida_Crear_TipoCuentaBanco crear_PuertoSalida;
    @Inject
    PuertoSalida_Actualizar_TipoCuentaBanco actualizar_PuertoSalida;
    @Inject
    PuertoSalida_Eliminar_TipoCuentaBanco eliminar_PuertoSalida;
    ...
    public Response getAll() {
        Controller_BuscarTodos_TipoCuentaBanco controlador = new Controller_BuscarTodos_TipoCuentaBanco(buscarTodos_PuertoSalida);
    ...
    public Response getById(@PathParam("id") long id) throws Exception {
        try {
            Controller_BuscarPorID_TipoCuentaBanco controlador = new Controller_BuscarPorID_TipoCuentaBanco(buscarPorID_PuertoSalida);
    ...
    public Response crear(Request_Save_DTO_TipoCuentaBanco dto) {
        try {
            Set<ConstraintViolation<Request_Save_DTO_TipoCuentaBanco>> violations = validator.validate(dto);
            ...
            Controller_Crear_TipoCuentaBanco controlador = new Controller_Crear_TipoCuentaBanco(crear_PuertoSalida);
    ...
    public Response fallbackCrear(Request_Save_DTO_TipoCuentaBanco dto) {
        return Response.status(503).build();
    }
    ...
    public Response actualizar(@PathParam("id") long id, Request_Update_DTO_TipoCuentaBanco dto) {
        try {
            Set<ConstraintViolation<Request_Update_DTO_TipoCuentaBanco>> violations = validator.validate(dto);
            if (!violations.isEmpty()) {
                List<String> errors = new ArrayList<>();
                violations.forEach(x -> errors.add(x.getMessage()));
                return Response.status(Response.Status.BAD_REQUEST).entity(new ValidationErrorResponse(errors)).build();
            ...
            Controller_Actualizar_TipoCuentaBanco controlador = new Controller_Actualizar_TipoCuentaBanco(actualizar_PuertoSalida);
    ...
    public Response fallbackActualizar(long id, Request_Update_DTO_TipoCuentaBanco dto) {
        return Response.status(503).build();
    }
    ...
    public Response delete(@PathParam("id") long id) {
        try {
            Controller_Eliminar_TipoCuentaBanco controlador = new Controller_Eliminar_TipoCuentaBanco(eliminar_PuertoSalida);
